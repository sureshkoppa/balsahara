...
$(function() {
        ...
        /*the folowing function is called when view Medical General register form is submitted*/
                $('form#um_adduser').submit(function(){
                    //alert('hi'+$("#gender").val);
                    ...
                    var user_pass_regex1= /[0-9]/;
                    var user_pass_regex2= /[a-z]/; 
                    var user_pass_regex3= /[A-Z]/;
                    /*****************REG EX*************************/
                    ...
                    /*validating the fields present in the form $("#refe_reg_profile_id")*/
                       if($("#userId").val().length<1)
                        ...
                        {
                            $dialog.html("User Id must be minimum 5 and maximun 20 character long.");
                            $dialog.parent().addClass( "ui-state-error" );
                            $dialog.dialog('open');
                            return false;
                        }
                        else if((!user_pass_regex.test($("#userId").val())))
                        {
                            $dialog.html("User Id can NOT contain space or special chracters except underscore '_'");
                            $dialog.parent().addClass( "ui-state-error" );
                            $dialog.dialog('open');
                            return false;
                        }
                       ...
                       /* else if((!user_pass_regex.test($("#userId").val())))
                        ...
                        else if((!user_pass_regex.test($("#password").val())))
                        {
                            $dialog.html("Password can NOT contain space or special chracters except underscore '_'");
                            $dialog.parent().addClass( "ui-state-error" );
                            $dialog.dialog('open');
                            return false;
                        }
                        else if(!user_pass_regex1.test($("#password").val())) 
                        { 
                           $dialog.html("password must contain at least one number (0-9)!");
                           $dialog.parent().addClass( "ui-state-error" );
                           $dialog.dialog('open');
                           return false;
                        } 
                        else if(!user_pass_regex2.test($("#password").val())) 
                        { 
                            $dialog.html("password must contain at least one lowercase letter (a-z)!"); 
                            $dialog.parent().addClass( "ui-state-error" );
                            $dialog.dialog('open');
                            return false; 
                        } 
                         
                        else if(!user_pass_regex3.test($("#password").val())) 
                        { 
                            $dialog.html("password must contain at least one uppercase letter (A-Z)!"); 
                            $dialog.parent().addClass( "ui-state-error" );
                            $dialog.dialog('open');
                            return false;
                        }
